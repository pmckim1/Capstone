...
class GuardianAccess(object):
    ...
    def search_and_download(self, session):
        for section in ["world"]:
            search_response = self.search(session, section=section, from_date="2015-01-01", to_date="2015-10-20")
            print(json.dumps(search_response, indent='\t'))

            with open('little_set29.csv', 'w', newline='', encoding="utf-8") as csvfile:
                ...
                for page in range(pages):
                    page = page + 1
                    search_response = self.search(session, section=section, from_date="2015-01-01", to_date="2015-10-20", page=page)
                    # print(json.dumps(search_response, indent='\t'))
                    if search_response:
